...
/**
 * {@code @RestController} 어노테이션이 선언된 클래스의 응답 본문을 가로채 {@link ApiResponse} 형식으로
 * 래핑 후 그 응답을 클라이언트에게 반환하는 클래스
 */
...
public class ApiResponseBodyAdvice implements ResponseBodyAdvice<Object> {
    ...
    public Object beforeBodyWrite(Object body, MethodParameter returnType, MediaType selectedContentType,
        ...
        return ApiResponse.ok((body == null) ? resolveEmptyPayload(actualType) : body);
    }

    /**
     * 응답 결과 데이터의 구조 유형에 따라 빈 리스트 또는 빈 맵을 반환한다.
     *
     * @param actualType    {@link ResponseEntity}가 감싸고 있던 데이터 타입
     * @return              {@code actualType}이 {@link List}일 경우 빈 리스트를, 아닐 경우 빈 객체를 반환
     */
    private Object resolveEmptyPayload(Type actualType) {
